...
public class WebServiceOAIPMHController {
    ...
    @RequestMapping(value = "/GetRecord", method = RequestMethod.POST, headers = {"Accept=application/xml"})
    public @ResponseBody
    ResponseEntity getRecordPost(ModelMap model,
                                 @ApiParam(value = "an argument that specifies the unique identifier of the item in the repository from which the record will be disseminated.") @RequestParam("identifier") String identifier,
                                 @ApiParam(value = "an argument that specifies the metadataPrefix of the format (e.g. \"oai_dc\") that will be included in the metadata part of the returned record.") @RequestParam("metadataPrefix") String metadataPrefix) {
        return getRecord( model, identifier, metadataPrefix);
        //return DigitalCommonsHelper.jsonToXml(webService.getRecordForIdentifierWebService(model, identifier));
        //return webService.getRecordForIdentifierWebService(model, identifier, metadataPrefix);
    }
    ...
    @RequestMapping(value = "/Identify", method = RequestMethod.POST, headers = "Accept=application/xml")
    public @ResponseBody
    ResponseEntity getIdentifyPost(ModelMap model) {
        return getIdentify(model);
    }
    ...
    @RequestMapping(value = "/ListIdentifiers", method = RequestMethod.POST, headers = "Accept=application/xml")
    public @ResponseBody
    ResponseEntity getIdentifiersPost(ModelMap model,
                                  @ApiParam(value = "an argument with a UTCdatetime value, which specifies a lower bound for datestamp-based selective harvesting") @RequestParam(value = "from", required = false) String from,
                                  @ApiParam(value = "an argument with a UTCdatetime value, which specifies a upper bound for datestamp-based selective harvesting.") @RequestParam(value = "until", required = false) String until,
                                  @ApiParam(required = true, value = "an argument, which specifies that headers should be returned only if the metadata format matching the supplied metadataPrefix is available or, depending on the repository's support for deletions, has been deleted. The metadata formats supported by a repository and for a particular item can be retrieved using the ListMetadataFormats request.") @RequestParam("metadataPrefix") String metadataPrefix,
                                  @ApiParam(value = "an argument with a setSpec value, which specifies set criteria for selective harvesting.") @RequestParam(value = "set", required = false) String set,
                                  @ApiParam(value = "an argument with a value that is the flow control token returned by a previous ListIdentifiers request that issued an incomplete list.") @RequestParam(value = "resumptionToken", required = false) String resumptionToken) {
        return  getIdentifiers(model, from, until, metadataPrefix, set, resumptionToken);
        //return webService.getIdentifiersWebService(model, from, until, metadataPrefix, set, resumptionToken);
    }
    ...
    @RequestMapping(value = "/ListRecords", method = RequestMethod.POST, headers = "Accept=application/xml")
    public @ResponseBody
    ResponseEntity getRecordsPost(ModelMap model,
                              @ApiParam(value = "an argument with a UTCdatetime value, which specifies a lower bound for datestamp-based selective harvesting") @RequestParam(value = "from", required = false) String from,
                              @ApiParam(value = "an  argument with a UTCdatetime value, which specifies a upper bound for datestamp-based selective harvesting.") @RequestParam(value = "until", required = false) String until,
                              @ApiParam(required = true, value = "an argument, which specifies that headers should be returned only if the metadata format matching the supplied metadataPrefix is available or, depending on the repository's support for deletions, has been deleted. The metadata formats supported by a repository and for a particular item can be retrieved using the ListMetadataFormats request.") @RequestParam("metadataPrefix") String metadataPrefix,
                              @ApiParam(value = "an argument with a setSpec value, which specifies set criteria for selective harvesting.") @RequestParam(value = "set", required = false) String set,
                              @ApiParam(value = "an argument with a value that is the flow control token returned by a previous ListIdentifiers request that issued an incomplete list.") @RequestParam(value = "resumptionToken", required = false) String resumptionToken) {
        return getRecords(model, from, until, metadataPrefix, set, resumptionToken);
        //return webService.getRecordsWebService(model, from, until, metadataPrefix, set, resumptionToken);
    }
    ...
    @RequestMapping(value = "/ListMetaDataFormats", method = RequestMethod.POST, headers = "Accept=application/xml")
    public @ResponseBody
        //ResponseEntity<String> getMetaDataFormats(ModelMap model, @RequestParam("identifier") Optional<String> identifier) {
    ResponseEntity getMetaDataFormatsPost(ModelMap model, @ApiParam(required = false, value = "an argument that specifies the unique identifier of the item for which available metadata formats are being requested. If this argument is omitted, then the response includes all metadata formats supported by this repository. Note that the fact that a metadata format is supported by a repository does not mean that it can be disseminated from all items in the repository.") @RequestParam(value = "identifier", required = false) Optional<String> identifier) {
        return getMetaDataFormats(model, identifier);
        //return webService.getMetadataFormatsWebService(model, identifier);
    }
    ...
    @RequestMapping(value = "/ListSets", method = RequestMethod.POST, headers = "Accept=application/xml")
    public @ResponseBody
    ResponseEntity getSetsWebServicePost(ModelMap model,
                                     @ApiParam(value = "an argument with a value that is the flow control token returned by a previous ListSets request that issued an incomplete list.") @RequestParam(value = "resumptionToken", required = false) String resumptionToken) {
        return getSetsWebService(model,resumptionToken);
        //return webService.getSetsWebService(model);
    }
