...
with open('isoAlpha2_to_Alpha3.xml') as f:
...
with open('continent_mapping.csv') as f:
...
with open('datasets.csv') as f:
    ...
    for row in new_dict:
        ...
        with open('tycho_dats_json/' + unique_id + '.json', 'w+') as out:
            out.write(json.dumps(dats_json, indent=4))

        with open('mdc_out.json', 'w+') as out:
